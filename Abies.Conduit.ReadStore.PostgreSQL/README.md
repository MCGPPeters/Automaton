...
- **Write side**: Domain events are persisted to KurrentDB (EventStoreDB) via `Automaton.Patterns.EventSourcing.KurrentDB`
...
Raw SQL implementations of the query capability delegates defined in `Abies.Conduit.ReadModel.Queries`:
...
- `Abies.Conduit` — Read model types and query capability delegates
- `Automaton.Patterns` — EventStore, StoredEvent, Projection abstractions
