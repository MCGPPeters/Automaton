...
namespace Automaton.Actor.Testing;
...
/// <remarks>
/// <para>
/// This helper intercepts the observer to capture state and events as they flow through
/// the actor's processing loop. It does NOT expose any additional API surface beyond what
/// the production <see cref="Address{TCommand}"/> provides — it just makes the effects
/// of commands observable for test assertions.
/// </para>
/// </remarks>
    ...
    where TDecider : Decider<TState, TCommand, TEvent, TEffect, TError, TParameters>
{
    ...
    /// </summary>
    /// <remarks>
    /// Counter-based drain: compares the number of commands sent via the address
    /// with the number processed by the loop. This avoids the race condition where
    /// the channel appears empty but dispatch is still in-flight.
    /// </remarks>
