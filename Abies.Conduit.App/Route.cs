...
// =============================================================================
// Pure functions for converting between URLs and application pages.
// Follows the Elm Architecture pattern where routing is a pure function
...
{
    /// <summary>
    /// Parses a <see cref="Url"/> into a <see cref="Page"/> with initial state.
    /// Returns the page and any commands needed to load data for that page.
    /// </summary>
    /// <param name="url">The URL to route.</param>
    /// <param name="session">The current user session (affects feed default).</param>
    /// <param name="apiUrl">The API base URL for constructing commands.</param>
    /// <returns>A tuple of the new page and commands to fetch initial data.</returns>
    ...
    /// <summary>
    /// Converts a <see cref="Page"/> back to a relative URL path.
    /// </summary>
    ...
    private static (Page, Command) HomeRoute(Session? session, string apiUrl)
    {
        ...
        {
            FeedTab.Your => (Command)new FetchFeed(apiUrl, session!.Token, Limit: Constants.ArticlesPerPage, Offset: 0),
            _ => new FetchArticles(apiUrl, session?.Token, Limit: Constants.ArticlesPerPage, Offset: 0)
        };
    ...
    private static (Page, Command) ProfileRoute(string username, bool showFavorites, string? token, string apiUrl)
    {
        ...
        var articleCmd = showFavorites
            ? (Command)new FetchArticles(apiUrl, token, Constants.ArticlesPerPage, 0, Favorited: username)
            : new FetchArticles(apiUrl, token, Constants.ArticlesPerPage, 0, Author: username);

        return (new Page.Profile(model), Commands.Batch(
            new FetchProfile(apiUrl, token, username),
            articleCmd));
    }
