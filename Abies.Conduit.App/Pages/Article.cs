// =============================================================================
// Article Page — Full Article View with Comments
// =============================================================================
// Displays a single article with markdown body, author info, action buttons,
// and a comment section with add/delete functionality.
// =============================================================================
...
public static class Article
{
    ...
    /// <summary>
    /// Renders the article banner with title and author meta.
    /// </summary>
    ...
    /// <summary>
    /// Renders the article meta section (author info + action buttons).
    /// </summary>
    ...
    /// <summary>
    /// Renders action buttons based on whether the current user is the author.
    /// </summary>
    private static Node[] ArticleActions(ArticleData article, Session? session)
    {
        if (session is not null && session.Username == article.Author.Username)
            ...
            [
                ...
                button([class_("btn btn-outline-danger btn-sm"),
                    onclick(new DeleteArticle(article.Slug))],
                [
        ...
        var followClass = article.Author.Following
            ? "btn btn-sm btn-secondary"
            : "btn btn-sm btn-outline-secondary";
        var followLabel = article.Author.Following
            ? $"\u2003Unfollow {article.Author.Username}"
            : $"\u2003Follow {article.Author.Username}";

        var favClass = article.Favorited
            ? "btn btn-sm btn-primary"
            : "btn btn-sm btn-outline-primary";
        var favLabel = article.Favorited
            ? $"\u2003Unfavorite Article ({article.FavoritesCount})"
            : $"\u2003Favorite Article ({article.FavoritesCount})";

        return
        [
            button([class_(followClass),
                onclick(new ToggleFollow(article.Author.Username, article.Author.Following))],
            [
                i([class_("ion-plus-round")], []),
                text(followLabel)
            ]),
            text("\u00A0\u00A0"),
            button([class_(favClass),
                onclick(new ToggleFavorite(article.Slug, article.Favorited))],
            [
                i([class_("ion-heart")], []),
                text(favLabel)
            ])
    ...
    /// <summary>
    /// Renders the comment form (only for logged-in users).
    /// </summary>
        ...
        [
            ...
            [
                textarea([
                    ...
                    rows("3"),
                    value(commentBody),
                    oninput(e => new CommentBodyChanged(e?.Value ?? ""))],
                    [])
            ]),
            div([class_("card-footer")],
            [
                img([src(session.Image ?? "https://api.realworld.io/images/smiley-cyrus.jpeg"),
                     class_("comment-author-img")]),
                button([class_("btn btn-sm btn-primary"), type("submit")],
                    [text("Post Comment")])
            ])
        ]);
    }
    ...
    private static Node CommentCard(CommentData comment, string slug, Session? session) =>
        ...
        [
            ...
            [
                ...
                span([class_("date-posted")],
                    [text(comment.CreatedAt.ToString("MMMM d, yyyy"))]),
                ..DeleteButton(comment, slug, session)
            ])
        ]);

    /// <summary>
    /// Renders the delete button only if the current user authored the comment.
    /// </summary>
    private static Node[] DeleteButton(CommentData comment, string slug, Session? session)
    {
        ...
        return
        [
            span([class_("mod-options")],
            [
                i([class_("ion-trash-a"),
                   onclick(new DeleteComment(slug, comment.Id))], [])
            ])
        ];
    }
