// =============================================================================
// DOM Diffing Benchmarks
// =============================================================================
// Measures the core virtual DOM diffing algorithm performance across:
//   • Small DOM trees (2-3 elements) — typical component updates
//   • Medium DOM trees (15-20 elements) — page section updates
//   • Large DOM trees (150+ elements) — data table updates
//   • Attribute-only and text-only changes
//   • Node addition and removal
//   • Keyed diffing: swap, reverse, shuffle (LIS algorithm)
//   • Dictionary key comparison (string vs int)
//
// Quality gates:
//   • Memory allocations should not increase by >10%
//   • Throughput should not decrease by >5%
// =============================================================================
...
public class DomDiffingBenchmarks
{
        ...
        [
            ...
            [
                ...
                [
                    ...
                    p([id("intro"), class_("highlight")], [text("Introduction paragraph")]),
                    p([id("body")], [text("Updated body paragraph")])
                ]),
                aside([id("sidebar")], [text("New sidebar")])
            ]),
            footer([id("footer")],
            [
    ...
    /// <summary>
    /// Small DOM tree diffing (2-3 elements). Typical component updates.
    /// </summary>
    [Benchmark]
    public void SmallDomDiff() => Operations.Diff(_smallOld, _smallNew);

    /// <summary>
    /// Medium DOM tree diffing (15-20 elements). Page section updates.
    /// </summary>
    [Benchmark]
    public void MediumDomDiff() => Operations.Diff(_mediumOld, _mediumNew);

    /// <summary>
    /// Large DOM tree diffing (150+ elements). Data table updates.
    /// </summary>
    [Benchmark]
    public void LargeDomDiff() => Operations.Diff(_largeOld, _largeNew);

    /// <summary>
    /// Attribute-only changes (no structural changes). Fastest diff scenario.
    /// </summary>
    [Benchmark]
    public void AttributeOnlyDiff() => Operations.Diff(_attrOld, _attrNew);

    /// <summary>
    /// Text content changes only. Common scenario for dynamic content updates.
    /// </summary>
    [Benchmark]
    public void TextOnlyDiff() => Operations.Diff(_textOld, _textNew);

    /// <summary>
    /// Node additions to existing tree. Tests append performance.
    /// </summary>
    [Benchmark]
    public void NodeAdditionDiff() => Operations.Diff(_addOld, _addNew);

    /// <summary>
    /// Node removals from existing tree. Tests removal detection performance.
    /// </summary>
    [Benchmark]
    public void NodeRemovalDiff() => Operations.Diff(_removeOld, _removeNew);
}
...
public class KeyedDiffingBenchmarks
{
    ...
    private void SetupSwap1k()
    {
        ...
        // Swap positions 1 and 998 (like js-framework-benchmark)
    ...
    /// <summary>
    /// Swap 2 rows in 1000 (js-framework-benchmark 05_swap1k).
    /// Tests keyed diffing with LIS optimization.
    /// </summary>
    [Benchmark(Baseline = true)]
    public void Swap1k() => Operations.Diff(_swap1kOld, _swap1kNew);

    /// <summary>
    /// Reverse 1000 rows. Worst case for keyed diffing — maximum reordering.
    /// </summary>
    [Benchmark]
    public void Reverse1k() => Operations.Diff(_reverse1kOld, _reverse1kNew);

    /// <summary>
    /// Random shuffle of 1000 rows. Tests keyed diffing with random reordering.
    /// </summary>
    [Benchmark]
    public void Shuffle1k() => Operations.Diff(_shuffle1kOld, _shuffle1kNew);
}

/// <summary>
/// Benchmarks for dictionary operations with string vs int keys.
/// Measures the overhead of string hashing in keyed diffing.
/// </summary>
[MemoryDiagnoser]
[MarkdownExporter]
public class DictionaryKeyBenchmarks
{
    private const int _elementCount = 1000;
    ...
    public void Setup()
    {
        _stringKeys = new string[_elementCount];
        _intKeys = new int[_elementCount];
        ...
        _stringDict = new Dictionary<string, int>(_elementCount);
        _intDict = new Dictionary<int, int>(_elementCount);
    ...
    {
        var dict = new Dictionary<string, int>(_elementCount);
        for (int i = 0; i < _elementCount; i++)
        {
    ...
    {
        var dict = new Dictionary<int, int>(_elementCount);
        for (int i = 0; i < _elementCount; i++)
        {
            dict[_intKeys[i]] = i;
        }
