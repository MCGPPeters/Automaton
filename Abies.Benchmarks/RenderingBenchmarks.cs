...
// =============================================================================
// Measures HTML rendering performance including:
//   • StringBuilder pooling for reduced allocations
//   • SearchValues<char> fast-path for HTML encoding
//   • Atomic counter for CommandId generation
//   • FrozenDictionary cache for event attribute names
//
// Scenarios covered:
//   • Simple elements (fast-path: no encoding needed)
//   • Elements requiring HTML encoding
//   • Elements with event handlers
//   • Small, medium, large page rendering
//   • Deeply nested and wide tree structures
//   • Complex forms with interactive elements
//
// Quality gates:
...
public class RenderingBenchmarks
{
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            [
                ...
                [
                    ...
                    [
                        Html.Elements.i([class_("ion-heart")], []),
                        text($" {i * 10}")
                    ])
                ]),
                ...
                [
                    ...
                    p([], [text($"This is the preview text for article {i + 1}. It contains a brief summary...")]),
                    span([], [text("Read more...")])
                ]),
    ...
    /// <summary>
    /// Simple element without HTML encoding needs. Fast path — most common case.
    /// </summary>
    [Benchmark(Baseline = true)]
    public string RenderSimpleElement() => Render.Html(_simpleElement);
    ...
    /// </summary>
    [Benchmark]
    public string RenderWithHtmlEncoding() => Render.Html(_encodingRequiredElement);
    ...
    /// </summary>
    [Benchmark]
    public string RenderWithEventHandlers() => Render.Html(_elementWithHandlers);

    /// <summary>
    /// Small page rendering (5-10 elements). Typical component size.
    /// </summary>
    [Benchmark]
    public string RenderSmallPage() => Render.Html(_smallPage);

    /// <summary>
    /// Medium page rendering (50 elements). Typical dashboard view.
    /// </summary>
    [Benchmark]
    public string RenderMediumPage() => Render.Html(_mediumPage);

    /// <summary>
    /// Large page rendering (200+ elements). Full Conduit home page.
    /// </summary>
    [Benchmark]
    public string RenderLargePage() => Render.Html(_largePage);

    /// <summary>
    /// Deeply nested tree (10 levels). Tests recursive rendering overhead.
    /// </summary>
    [Benchmark]
    public string RenderDeeplyNested() => Render.Html(_deeplyNested);

    /// <summary>
    /// Wide tree (50 siblings). Tests iteration performance.
    /// </summary>
    [Benchmark]
    public string RenderWideTree() => Render.Html(_wideTree);

    /// <summary>
    /// Complex form with many interactive elements. Combined stress test.
    /// </summary>
    [Benchmark]
    public string RenderComplexForm() => Render.Html(_complexForm);
}
