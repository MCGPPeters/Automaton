...
// - Clear all rows
//
// This implementation uses the Automaton kernel's MVU runtime.
...
/// <summary>Swap rows at positions 2 and 998 (1-indexed, so indices 1 and 997).</summary>
...
/// <summary>
/// The js-framework-benchmark application implementing the Automaton MVU pattern.
/// </summary>
public sealed class BenchmarkApp : Program<BenchmarkModel, Unit>
{
    ...
    /// </summary>
    public static (BenchmarkModel State, Command Effect) Initialize(Unit argument) =>
        (new BenchmarkModel([], null, 1), Commands.None);

    /// <summary>
    /// Transition the model based on incoming messages.
    /// </summary>
    public static (BenchmarkModel State, Command Effect) Transition(BenchmarkModel state, Message message) =>
        message switch
        {
            Create => (state with
            {
                Data = BuildData(1000, state.NextId),
                NextId = state.NextId + 1000,
                Selected = null
            }, Commands.None),

            CreateLots => (state with
            {
                Data = BuildData(10000, state.NextId),
                NextId = state.NextId + 10000,
                Selected = null
            }, Commands.None),

            Append => (state with
            {
                Data = [.. state.Data, .. BuildData(1000, state.NextId)],
                NextId = state.NextId + 1000
            }, Commands.None),

            UpdateEvery10th => (state with { Data = UpdateEvery10thRow(state.Data) }, Commands.None),

            Clear => (state with { Data = [], Selected = null }, Commands.None),

            SwapRows => (state with { Data = SwapRowsAtPositions(state.Data) }, Commands.None),

            Select s => (state with { Selected = s.Id }, Commands.None),

            Delete d => (state with { Data = state.Data.Where(r => r.Id != d.Id).ToList() }, Commands.None),

            _ => (state, Commands.None)
        };
