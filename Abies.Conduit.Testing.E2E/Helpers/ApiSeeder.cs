...
// =============================================================================
// Creates test data via the Conduit REST API endpoints.
// Follows Playwright best practice §7.1: "Seed test data via dedicated
// test APIs or internal endpoints."
//
// Since the Conduit API follows the standard RealWorld spec, we use the
// public endpoints for seeding: register users, create articles, add comments.
// This ensures seeded data flows through the same validation pipeline as
// real user actions.
// =============================================================================
...
public sealed class ApiSeeder
{
    ...
    // The Conduit API uses event sourcing with async read-model projection.
    // After writing data (register, create article, etc.), the read model
    // may not have caught up yet. These helpers poll until the data is
    // available, ensuring tests don't fail due to eventual consistency lag.
