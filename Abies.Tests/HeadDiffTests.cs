...
public class HeadDiffTests
{
    ...
    public void Diff_CanonicalHrefChanged_ReturnsUpdate()
    {
        ...
        // Canonical changes href so key changes → old removed, new added
        Assert.Equal(2, patches.Count);
    ...
    public void Diff_MixedChanges_ReturnsCorrectPatches()
    {
        HeadContent[] oldHead =
        [
            Head.meta("description", "Old desc"),       // will be updated
            Head.og("title", "Title"),                   // will be removed
            Head.stylesheet("/style.css")                // unchanged
        ];

        HeadContent[] newHead =
        [
            Head.meta("description", "New desc"),        // update
            Head.stylesheet("/style.css"),               // unchanged
            Head.canonical("https://example.com")        // new (add)
        ];

        var patches = HeadDiff.Diff(oldHead, newHead);

        // Expect: 1 update (description), 1 add (canonical), 1 remove (og:title)
